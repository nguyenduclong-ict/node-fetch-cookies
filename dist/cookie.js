...
const validateHostname = (cookieHostname, requestHostname, subdomains) => {
    ...
    if (requestHostname === cookieHostname ||
...
class Cookie {
    constructor(str, requestURL) {
        if (typeof str !== "string")
            throw (0, errors_1.paramError)("First", "str", "new Cookie()", "string");
        if (typeof requestURL !== "string")
            throw (0, errors_1.paramError)("Second", "requestURL", "new Cookie()", "string");
        // parse url
        // if it is invalid an error will be raised
        const parsedURL = new url_1.default.URL(requestURL);
        const splitted = str.split(/;\s*/);
        [this.name, this.value] = splitN(splitted[0], "=", 1);
        if (!this.name)
            throw new errors_1.CookieParseError('Invalid cookie name "' + this.name + '"!');
            ...
            if (v) {
                if (k === "expires") {
                    ...
                    if (!/^(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun), \d{2}[ -](?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[ -]\d{2,4} \d{2}:\d{2}:\d{2} GMT$/.test(v) ||
                        ...
                        throw new errors_1.CookieParseError('Invalid value for Expires "' + v + '"!');
                }
                else if (k === "max-age") {
                    const seconds = ~~+v;
                    if (seconds.toString() !== v)
                        throw new errors_1.CookieParseError('Invalid value for Max-Age "' + v + '"!');
                    ...
                    if (!validateHostname(v, parsedURL.hostname, true))
                        throw new errors_1.CookieParseError('Invalid value for Domain "' +
                            v +
                    ...
                    throw new errors_1.CookieParseError('Invalid key "' +
                        k +
                        '" with value "' +
                        v +
                        '" specified!');
                ...
                if (k === "secure")
                    ...
                    throw new errors_1.CookieParseError('Invalid key "' + k + '" specified!');
            }
        }
        if (this.name.toLowerCase().startsWith("__secure-") &&
            (!this.secure || parsedURL.protocol !== "https:"))
            throw new errors_1.CookieParseError('Cookie has "__Secure-" prefix but "Secure" isn\'t set or the cookie is not set via https!');
        if (this.name.toLowerCase().startsWith("__host-") &&
            (!this.secure ||
                parsedURL.protocol !== "https:" ||
                this.domain ||
                this.path !== "/"))
            throw new errors_1.CookieParseError('Cookie has "__Host-" prefix but "Secure" isn\'t set, the cookie is not set via https, "Domain" is set or "Path" is not equal to "/"!');
        // assign defaults
        if (!this.domain) {
    ...
    hasExpired(sessionEnded) {
        return ((sessionEnded && this.expiry === null) ||
